		...
		if (resultCode == Activity.RESULT_OK) {
			if (srcType == CAMERA) {			
				...
				try {
					...
					//Customization Starts - Tavares 22/09/17
    				...
    				int scaleFactor = Math.min(photoW/this.targetWidth, photoH/this.targetHeight);
					// Decode the image file into a Bitmap sized to fill the View
					options.inJustDecodeBounds = false;
					options.inSampleSize = scaleFactor;
					options.inPurgeable = true;

					Bitmap bitmap2 = BitmapFactory.decodeFile(this.imageUri.getPath(), options);					
					if (options.outWidth == -1 || options.outHeight == -1) {
						this.failPicture("Error decoding image.");
						return;
					}
					//Orientation Image
					//Samsung and LG Devices rotates image
					ExifInterface exif = new ExifInterface(this.imageUri.getPath());
					...
					//re-write the file 
					FileOutputStream out = new FileOutputStream(this.imageUri.getPath());	
					bitmap2.compress(Bitmap.CompressFormat.JPEG, this.mQuality, out);

					//Customization End

					JSONObject returnObject = new JSONObject();
					returnObject.put("url", this.imageUri.toString());
					returnObject.put("rotate", rotate);
					returnObject.put("rotation", orientation);
